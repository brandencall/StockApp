...
begin
    ...
    SELECT
        ...
        MAX(CASE WHEN fa.Title = 'NetCashProvidedByUsedInInvestingActivities' THEN cv.Value END) AS NetCashProvidedByUsedInFinancingActivities
    FROM
        DistinctStocks ds
    ...
    WHERE
        fa.Title IN (
            'MarketCapitalization','NetCashProvidedByUsedInInvestingActivities'
        )
        AND (ff.Form = '10-K' OR ff.Form IS NULL)
        AND ff.EndDate LIKE '%' + @year + '%'
        AND ff.FiscalYear = @year
